...
public class GiftDialog : Dialog {
    ...
    {
#if UNITY_EDITOR
        GameState.hint.ChangeValue(2);
        Close();
#else
        if (!AdmobManager.instance.ShowRewardedAd())
        {
            Toast.instance.ShowMessage("Ad is not available now, please try again..", 2f);
        }
#endif
    }
