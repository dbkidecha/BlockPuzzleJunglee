...
public class TileRegion : MonoBehaviour
{
    ...
    private Dictionary<Vector2, Block> slots = new Dictionary<Vector2, Block>();
    ...
    private void LoadBoardBackground()
    {
        for (int col = 0; col < size.y; col++)
        {
            ...
            {
                Block tile = Instantiate(MonoUtils.instance.tile_background);
                tile.transform.SetParent(transform);
                tile.transform.localScale = Vector3.one;
                Vector3 position = col % 2 == 0 ? new Vector3(col * 1.5f * Block.EDGE_SIZE, row * Block.HEIGHT) :
                        new Vector3(col * 1.5f * Block.EDGE_SIZE, row * Block.HEIGHT + Block.HEIGHT / 2);
    ...
    public void Load(GameLevel gameLevel)
    {
        ...
        foreach (var value in positions)
        {
            ...
            Block tile = Instantiate(MonoUtils.instance.tile_background2);
        ...
        foreach (var value in positions)
        {
            ...
            Block tile = Instantiate(values.Length == 2 ? MonoUtils.instance.tile_background : MonoUtils.instance.tile_stone);
            tile.transform.SetParent(MonoUtils.instance.backgroundTilesTransform);
            tile.transform.localScale = Vector3.one;
            Vector3 position = GetLocalPosition(col, row);
            tile.transform.localPosition = position;
        ...
        float regionWidth = maxX - minX + Block.WIDTH + (minX - Block.WIDTH / 2) * 2; ;
        float regionHeight = maxY - minY + Block.HEIGHT + (minY - Block.HEIGHT / 2) * 2;
    ...
    private void LoadPieces(GameLevel gameLevel)
    {
        ...
        foreach (var oneData in data)
        {
            ...
            {
                Block tile = Instantiate(MonoUtils.instance.tile_background_bottom);
    ...
    private Piece CreatePiece(List<Vector2> positions, Transform parent)
    {
        ...
        foreach (var position in piece.defaultPositions)
        {
            ...
            Block tile = Instantiate(MonoUtils.instance.tile_normal);
            ...
            // Add shadows
            Block tileS = Instantiate(MonoUtils.instance.tile_shadow);
    ...
    {
        return col % 2 == 0 ? new Vector3((col * 1.5f + 1) * Block.EDGE_SIZE, (row + 0.5f) * Block.HEIGHT) :
                        new Vector3((col * 1.5f + 1) * Block.EDGE_SIZE, (row + 1) * Block.HEIGHT);
    }
    ...
    public bool CheckMatch(Piece piece)
    {
        ...
        Block matchSlot = null;

        foreach (Block slot in slots.Values)
        {
    ...
    public void CheckGameComplete()
    {
        ...
        if (isComplete)
        {
            ...
            List<Block> fadedTiles = GetTileOnBoard();
    ...
    private List<Block> GetTileOnBoard()
    {
    ...
    private IEnumerator FadeTiles(List<Block> fadedTiles)
    {
        foreach (Block tile in fadedTiles)
        {
    ...
    public class TileComparer : IComparer<Block>
    {
        ...
        double F(Block a) // your calculation
        {
