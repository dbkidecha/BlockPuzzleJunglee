...
public class TileRegion2 : MonoBehaviour {
    ...
    private Dictionary<Vector2, Block> bottomSlots = new Dictionary<Vector2, Block>();
    ...
    public void LoadBoardBackground()
    {
        ...
        gameLevel = LevelMaker.instance.gameLevel;
        float minX = float.MaxValue, minY = float.MaxValue, maxX = float.MinValue, maxY = float.MinValue;

        if (string.IsNullOrEmpty(gameLevel.positions))
        {
            for (int col = 0; col < LevelMaker.instance.numCol; col++)
            {
                for (int row = 0; row < LevelMaker.instance.numRow; row++)
                {
                    ...
                    tile.type = Block.Type.Background;
    ...
    public bool CheckMatch(Piece2 piece)
    {
        ...
        foreach(Block slot in bottomSlots.Values)
        {
